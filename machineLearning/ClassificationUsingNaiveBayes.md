...
```r
#min.freq initial settings -> around 10% of the number of docs in the corpus
...
Another interesting visualization involves comparing teh clouds of SMS __spam__ and __ham__ using the raw text messages ... 

```r
rawData_train_spam <- subset(train_dataset, type == "spam")
rawData_train_ham <- subset(train_dataset, type == "ham")
```
...
![](ClassificationUsingNaiveBayes_files/figure-html/wordCloudRawTrainigSpam-1.png) 
...
![](ClassificationUsingNaiveBayes_files/figure-html/wordCloudRawTrainigHam-1.png) 
...
###Creating indicator features for frequnet words
Transform the sparse matrix into a data structure that can be used to train a __naive Bayes classifier__.
...
## Weighting          : term frequency (tf)
```
 The sparse matrix used for training include 3891 documents and 7868 features (terms). Not all of these terms will be useful for classification. In order to reduce thenumber of features we can proceed to consider the words that appears at least a certain number of times...
...
To limit our training and test matrix to only the words in the dictionary we can use the following commands ...
...
```r
sms_classifier <- naiveBayes(sms_train, train_dataset$type)
```
...
table(test_dataset$type, sms_test_pred)
##       sms_test_pred
##         ham spam
##   ham  1438    6
##   spam   29  195
