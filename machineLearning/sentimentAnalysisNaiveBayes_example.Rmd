...
```{r enrichData}
#Add a sentiment feature
rawData_s$sentiment <- rawData_s$type == "Sentiment"

#Sentiment Relevant Data
table(rawData_s$sentiment)
```
...
There are 1553 terms/ features idendified as frequent terms. __To limit our training and test matrix to only the words in the dictionary of frequent terms__ we can use the following commands ...
...
```{r trainTheModel}
sentiment_classifier <- naiveBayes(trainingDtm_dic, trainingData$sentiment,, laplace = 1)
```
##Test the model against the test dataset and evaluate model performance

```{r testTheModel, collapse=TRUE}
test_pred <- predict(sentiment_classifier, testDtm_dic, threshold = 0.05)
```

```{r evaluateTheModel, collapse=TRUE}
#table actual (row) vs. predicted (col)
table(testData$sentiment, test_pred)
```
